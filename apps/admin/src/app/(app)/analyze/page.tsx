...
export default function AnalyzePage() {
	...
	// Score-driven coaching tone. The Analyzer's own system prompt says
	// "evaluate strengths first, never start with negativity" — so at high
	// scores the UI should stop listing "problems" and reframe suggestions
	// as optional polish.
	//   ≥ 90  → hide problems + captionIssues entirely, reframe labels.
	//   80-89 → cap each list to the single most important item.
	//   < 80  → full critique (those really do need it).
	const score = results?.score ?? 0;
	const isVeryStrong = score >= 90;
			...
			<div className="mt-10">
				{analyze.isPending ? (
					...
					<div className="flex flex-col gap-6">
						...
						{/* Very-Strong reassurance — only at ≥ 90, replaces the critique tone. */}
						{isVeryStrong ? (
							<motion.div
							...
							>
								...
								<div className="flex-1">
									<p className="text-[14px] font-semibold text-text-primary">
										This already scores Very Strong.
									</p>
									<p className="mt-0.5 text-[13px] leading-relaxed text-text-secondary">
										The suggestions below are optional refinements, not fixes.
									</p>
								</div>
							</motion.div>
						) : null}

						{/* Quick fix — eye-catching (labelled "Polish tip" at ≥ 90) */}
						<motion.div
							...
							<Card className="flex items-start gap-4 p-5">
								...
								<div className="flex-1">
									<SectionLabel className="mb-1.5">
										{isVeryStrong ? "Polish tip" : "Quick fix"}
									</SectionLabel>
						...
						>
							...
							>
								<div className="mb-2 flex items-center justify-between">
									<SectionLabel accent>
										{isVeryStrong ? "Sharper variant" : "Better hook"}
									</SectionLabel>
									{!isVeryStrong && results.priorityFix === "hook" ? (
						...
						{/* Problems + caption issues — hidden at ≥90, capped at 1 item when 80–89. */}
						{!isVeryStrong ? (
							<motion.div
								initial={{ opacity: 0, y: 10 }}
								animate={{ opacity: 1, y: 0 }}
							...
							>
								<Card className="p-5">
									...
									<div className="flex flex-wrap gap-2">
										{(isStrong ? results.problems.slice(0, 1) : results.problems).map((p) => (
											<span
												key={p}
								...
								<Card className="p-5">
									...
									<ul className="flex flex-col gap-2">
										{(isStrong ? results.captionIssues.slice(0, 1) : results.captionIssues).map((issue) => (
											<li
												key={issue}
						...
						>
							...
							>
								<div className="mb-3 flex items-center justify-between">
									<SectionLabel accent={!isVeryStrong && results.priorityFix === "caption"}>
										{isVeryStrong ? "Alternate variant" : "Improved caption"}
									</SectionLabel>
									{!isVeryStrong && results.priorityFix === "caption" ? (
										<span className="rounded-full bg-accent px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white">
											🎯 Priority
										</span>
									) : null}
